/**
 * Marks feedback rows and changelog entries shipped after a PR is confirmed merged into dev.
 *
...
export async function runShip(options: RunShipOptions): Promise<void> {
  ...
  await markFeedbackShippedByPr(options.prNumber, {
    versionResolved: options.versionResolved,
  })
  await shipChangelogEntryByPrNumber(options.prNumber)
}
...
async function main(): Promise<void> {
  ...
  if (prIndex === -1 || !args[prIndex + 1]) {
    process.stderr.write('Usage: steward:ship -- --pr <number> --version <semver>\n')
    process.exit(1)
  }
  ...
  process.stderr.write(`PR #${prNumber} shipped — rows and changelog marked shipped at version ${versionResolved}\n`)
}
