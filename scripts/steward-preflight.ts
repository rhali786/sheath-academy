...
async function main(): Promise<void> {
  ...
  process.stderr.write(
    [
      `Steward preflight started at ${result.startedAt}`,
      `DB query ok — submitted rows: ${result.submittedCount}`,
      `Claude preflight ok in ${result.claude.elapsedMs}ms (timeout ${result.claude.timeoutMs}ms)`,
      result.sampleIds.length > 0 ? `Sample feedback ids: ${result.sampleIds.join(', ')}` : 'Sample feedback ids: none',
    ].join('\n') + '\n',
  )
  process.exit(0)
}
