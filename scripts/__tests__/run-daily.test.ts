...
describe('runDaily', () => {
  ...
  it('daily planning uses a shorter default Claude timeout', async () => {
    mockListEligibleFeedbackForDailyRun.mockResolvedValue({
      feedbackIds: ['fb_1'],
      autoEligibleIds: ['fb_1'],
      approvedIds: [],
  ...
  it('daily planning honors STEWARD_CLAUDE_PLAN_TIMEOUT_MS override', async () => {
    const previousTimeout = process.env.STEWARD_CLAUDE_PLAN_TIMEOUT_MS
    process.env.STEWARD_CLAUDE_PLAN_TIMEOUT_MS = '15000'
    mockListEligibleFeedbackForDailyRun.mockResolvedValue({
      feedbackIds: ['fb_1'],
      autoEligibleIds: ['fb_1'],
      approvedIds: [],
    ...
    try {
      ...
      if (previousTimeout === undefined) {
        delete process.env.STEWARD_CLAUDE_PLAN_TIMEOUT_MS
      } else {
        process.env.STEWARD_CLAUDE_PLAN_TIMEOUT_MS = previousTimeout
      }
    }
