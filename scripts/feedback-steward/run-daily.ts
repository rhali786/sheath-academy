...
import { STEWARD_MODEL_PLAN, STEWARD_MODEL_EXECUTE } from './steward-models'
    ...
    workstreams: {
      ...
      items: {
        ...
        properties: {
          ...
          owningComponent: { type: 'string', minLength: 1 },
          allowedFiles: {
            type: 'array',
            minItems: 1,
            items: { type: 'string', minLength: 1 },
        ...
        required: ['featureArea', 'summary', 'owningComponent', 'allowedFiles', 'testPlan', 'blastRadiusNotes', 'clarifyingQuestions', 'uatByFeedbackId'],
      },
    },
  },
...
function validateWorkstream(value: unknown): DailyPlanWorkstream | null {
  ...
  if (typeof value.owningComponent !== 'string' || value.owningComponent.trim().length === 0) return null
  if (!isStringArray(value.allowedFiles, { minItems: 1 })) return null
  if (!isStringArray(value.testPlan, { minItems: 1 })) return null
  if (typeof value.blastRadiusNotes !== 'string') return null
  if (!Array.isArray(value.clarifyingQuestions)) return null
  if (!value.clarifyingQuestions.every((q: unknown) => typeof q === 'string' && q.trim().length > 0)) return null
  ...
  return {
    ...
    owningComponent: value.owningComponent.trim(),
    allowedFiles: value.allowedFiles.map((entry) => entry.trim()),
    testPlan: value.testPlan.map((entry) => entry.trim()),
    blastRadiusNotes: value.blastRadiusNotes.trim(),
    clarifyingQuestions: (value.clarifyingQuestions as string[]).map((q) => q.trim()),
    uatByFeedbackId,
  }
}
...
function loadDoNotAutomateConfig(): DoNotAutomateConfig {
  const configPath = path.join(process.cwd(), 'scripts', 'feedback-steward', 'do-not-automate.json')
...
function generateDailyPlan(eligibility: DailyRunEligibilityResult, generatedAt: string): DailyPlanArtifact {
  const skillPath = path.join(process.cwd(), '.claude', 'skills', 'feedback-daily-plan', 'SKILL.md')
  const skillPrompt = readFileSync(skillPath, 'utf8')
  process.stderr.write(
    `Generating daily plan with Claude for ${eligibility.feedbackIds.length} feedback items at ${new Date().toISOString()}...\n`,
  )
...
function executeDailyPlan(
  ...
  const skillPath = path.join(process.cwd(), '.claude', 'skills', 'feedback-execute', 'SKILL.md')
  const skillPrompt = readFileSync(skillPath, 'utf8')
  process.stderr.write(`Executing reviewed plan with Claude at ${new Date().toISOString()}...\n`)
...
export function renderDailyPlanMarkdown(plan: DailyPlanArtifact, eligibility: DailyRunEligibilityResult): string {
  ...
  const sections = plan.workstreams.map((workstream, index) => {
    ...
    const blastSection = workstream.blastRadiusNotes
      ? [`- Blast radius: ${workstream.blastRadiusNotes}`]
      : []

    const questionsSection = workstream.clarifyingQuestions.length > 0
      ? [
          ``,
          `> ⚠️ **Clarifying questions — confirm before executing this workstream:**`,
          ...workstream.clarifyingQuestions.map((q) => `> - ${q}`),
        ]
      : []
    ...
    return [
      ...
      `- Owning component: ${workstream.owningComponent}`,
...
export async function runDaily(options: RunDailyOptions): Promise<RunDailyResult> {
  ...
  await classifyFeedback(options.feedbackIds ? { ids: options.feedbackIds } : {})

  const eligibility = await listEligibleFeedbackForDailyRun(config, options.feedbackIds)
...
async function main(): Promise<void> {
  ...
  const workstreamCount = result.plan.workstreams.length
  const feedbackCount = result.plan.feedbackIds.length
  const mode = result.dryRun ? 'plan-only' : 'plan + execute'

  process.stderr.write(
    [
      '',
      `✨ Steward daily run complete (${mode})`,
      '',
      ...
      ...(result.execution
        ? [
            ...
            result.execution.previewUrl ? `🌐 Preview: ${result.execution.previewUrl}` : '',
          ].filter(Boolean)
        : [`💡 Review the markdown plan, then run: npm run steward:execute -- --artifact ${result.jsonArtifactPath}`]),
      '',
    ].join('\n')
  )
}
