...
import type { FeedbackRequeueItem } from './feedback-requeue'
...
function dedupeAgainstExisting(
  candidate: { id: string; message: string | null },
  existingRows: FeedbackRow[]
): DedupeResult {
  if (candidate.message && candidate.message.trim().length > 0) {
    ...
    for (const row of existingRows) {
      ...
      if (jaccardSimilarity(candidateTokens, tokenize(row.message)) >= SIMILARITY_THRESHOLD) {
...
export async function detectDuplicatesBatch(
  candidates: FeedbackRequeueItem[]
  ...
  for (const candidate of candidates) {
    results.set(candidate.id, dedupeAgainstExisting(candidate, existingRows))
  }
...
export async function detectDuplicate(candidateId: string): Promise<DedupeResult> {
  ...
  if (!candidate) return { isDuplicate: false, duplicateOfId: null, reason: null }
  ...
  if (candidate.prNumber !== null) {
    ...
    if (prMatch) return { isDuplicate: true, duplicateOfId: prMatch.id, reason: 'matching_pr_number' }
  }

  return dedupeAgainstExisting(candidate, existingRows)
}
...
if (require.main === module) {
  ...
  if (!id) {
    process.stderr.write('Usage: tsx scripts/feedback-steward/feedback-dedupe.ts --id <feedbackId>\n')
    process.exit(1)
  }
