...
- **`steward:daily` performs classification automatically.** It classifies submitted rows before checking eligibility and generating the plan. Running the standalone intake scripts first is optional and now mainly useful for debugging.
...
3. **Expected output:**
   ...
   - The row can be identified by its `id`, message text, and user email
...
6. Check the row in the database or via the admin UI:
   ...
   - High-confidence, low-risk rows should move directly to `classified`
   - Lower-confidence or riskier rows should move to `awaiting_approval`
