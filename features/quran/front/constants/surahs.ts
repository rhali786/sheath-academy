export const SURAHS = [
  ...
  { number: 23, name: 'Al-Muminun' },
  { number: 24, name: 'An-Nur' },
  { number: 25, name: 'Al-Furqan' },
  { number: 26, name: 'Ash-Shuara' },
...
export function getSurahDisplay(number: number): string {
  const surah = SURAHS.find(s => s.number === number)
  return surah ? `${number} - ${surah.name}` : `${number}`
}
