...
description: Use when writing tests for Sheath Academy — unit, API route handler, Postgres repository, or integration (component-with-context) tests. Provides copy-paste boilerplate for each test type and the key Jest rules (mock at the repository boundary, never mock getDb(), mockImplementation not mockReturnValueOnce, Nivo/next-auth auto-mocks).
---

# Testing patterns

Concrete boilerplate for the test types used in this codebase. For *which* tests a plan must include and TDD ordering, use the `plan-builder` skill.
...
jest.mock('@/features/<feature>/front/context', () => ({ use<Feature>: jest.fn() }))
...
const defaultHousehold = () => ({
  familyName: 'Test Family', householdProfile: null, needsSetup: false,
  loading: false, error: null, refetch: jest.fn(),
})
const defaultFeature = () => ({ items: [], loading: false, error: null })
...
describe('MyComponent', () => {
  ...
  it('renders populated state', () => {
    mockUse<Feature>.mockImplementation(() => ({
      ...defaultFeature(), items: [{ id: '1', title: 'Thing' }],
    }))
    render(<MyComponent />)
...
const makeThing = (overrides: Partial<Thing> = {}): Thing => ({
  id: 'thing_001', title: 'Test Thing', status: 'active',
